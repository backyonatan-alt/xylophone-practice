...
## Phase 3 (color matching — parents match the app to their own toy)

Full spec: `color-settings-prd.md`; design sources: `design/*.dc.html`.

- The bar palette is configurable: a one-time first-visit setup screen ("האם הקסילופון שלכם נראה כך?"), then a settings screen (sliders button on the library header) with a flip-direction toggle, visual presets, and per-bar manual swatches with a duplicate-color warning. Bar labels/notes never change — only colors.
- `BARS` in `app.js` stays the single source of truth and is derived at startup: `?colors=` URL param (a preset id or `x-RRGGBB-…-RRGGBB`; wins for the visit, never persisted) → localStorage `xyloColors` → default. `buildQuery` carries `colors` across navigation.
- Every assignable color has a measured WCAG-AA label color in `COLOR_FG` — if you add a color (swatch or preset), measure its fg too. Unknown stored hexes fall back to computed white/ink contrast.
- OG images and favicons stay in the default palette (`BAR_COLORS` in build.js) — they are build-time marketing assets, not notation.
- The "metal" and "pastel" preset palettes are first guesses — verify against the actual best-selling toys and adjust `PRESETS` if needed.
